...
// If you want to pass lua_State to the functions then define this value at the
// project level.
...
const char *RunString(
    ...
    const char *szLua) {
  if (!lState) {
    fprintf(stderr, "You didn't pass a valid lState, either assign that or "
                    "refactor LoadImguiBindings and RunString\n");
  }
...
#define IM_VEC_4_ARG(name)                                                     \
  ...
  const ImVec4 name((double)i_##name##_x, (double)i_##name##_y,                \
                    (double)i_##name##_z, (double)i_##name##_w);
...
#define IMGUI_FUNCTION_DRAW_LIST(name)                                         \
  {"DrawList_" #name, impl_draw_list_##name},
...
void LoadImguiBindings(lua_State *lState) {
  if (!lState) {
    fprintf(stderr, "You didn't assign the global lState, either assign that "
                    "or refactor LoadImguiBindings and RunString\n");
  }
  lua_newtable(lState);
