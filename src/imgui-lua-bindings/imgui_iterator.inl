...
//    style editor block (not a window). you can pass in a reference ImGuiStyle
...
//    style selector block (not a window), essentially a combo listing the
//    default styles.
...
//    font selector block (not a window), essentially a combo listing the loaded
//    fonts.
...
//    basic help/info block (not a window): how to manipulate ImGui as a
...
//    the compiled version string e.g. "1.80 WIP" (essentially the value for
...
//    = ImVec2 0  0, bool border = false, ImGuiWindowFlags flags = 0);
// Unsupported arg type  const ImVec2& size = ImVec2 0  0
//    IMGUI_API bool          BeginChild(ImGuiID id, const ImVec2& size = ImVec2
//    0  0, bool border = false, ImGuiWindowFlags flags = 0);
// Unsupported arg type  const ImVec2& size = ImVec2 0  0
...
//    current window focused? or its root/child, depending on flags. see flags
//    for options.
// Unsupported arg type ImGuiFocusedFlags flags=0
//    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is
//    current window hovered (and typically: not blocked by a popup/modal)? see
//    flags for options. NB: If you are trying to check whether your mouse
...
//    draw list associated to the current window, to append your own drawing
//    primitives
// Unsupported return type ImDrawList*
//    IMGUI_API ImVec2        GetWindowPos();                             // get
//    current window position in screen space (useful if you want to do your own
...
//    current window width (shortcut for GetWindowSize().x)
IMGUI_FUNCTION(GetWindowWidth)
CALL_FUNCTION(GetWindowWidth, float)
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API float         GetWindowHeight();                          // get
//    current window height (shortcut for GetWindowSize().y)
...
//    = 0, const ImVec2& pivot = ImVec2 0  0); // set next window position. call
//    before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
// Unsupported arg type  ImGuiCond cond = 0
// Unsupported arg type  const ImVec2& pivot = ImVec2 0  0
//    IMGUI_API void          SetNextWindowSize(const ImVec2& size, ImGuiCond
//    cond = 0);                  // set next window size. set axis to 0.0f to
//    force an auto-fit on this axis. call before Begin()
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          SetNextWindowSizeConstraints(const ImVec2&
//    size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback =
//    NULL, void* custom_callback_data = NULL); // set next window size limits.
//    use -1,-1 on either X/Y axis to preserve the current size. Sizes will be
...
//    bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic.
//    call before Begin()
...
//    cond = 0);                 // set next window collapsed state. call before
//    Begin()
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          SetNextWindowFocus(); // set next window to be
//    focused / top-most. call before Begin()
IMGUI_FUNCTION(SetNextWindowFocus)
CALL_FUNCTION_NO_RET(SetNextWindowFocus)
END_IMGUI_FUNC
//    IMGUI_API void          SetNextWindowBgAlpha(float alpha); // set next
//    window background color alpha. helper to easily override the Alpha
...
//    0);                        // (not recommended) set current window
//    position - call within Begin()/End(). prefer using SetNextWindowPos(), as
//    this may incur tearing and side-effects.
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond =
//    0);                      // (not recommended) set current window size -
//    call within Begin()/End(). set to ImVec2 0  0 to force an auto-fit. prefer
...
//    window to be focused / top-most. prefer using SetNextWindowFocus().
IMGUI_FUNCTION(SetWindowFocus)
CALL_FUNCTION_NO_RET(SetWindowFocus)
END_IMGUI_FUNC
//    IMGUI_API void          SetWindowFontScale(float scale); // set font
//    scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is
//    an old API! For correct scaling, prefer to reload font + rebuild
...
//    size, ImGuiCond cond = 0);    // set named window size. set axis to 0.0f
//    to force an auto-fit on this axis.
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          SetWindowCollapsed(const char* name, bool
//    collapsed, ImGuiCond cond = 0);   // set named window collapsed state
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          SetWindowFocus(const char* name); // set named
//    window to be focused / top-most. use NULL to remove focus.
...
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetContentRegionMax(); // current content
//    boundaries (typically window boundaries including scrolling, or current
...
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetWindowContentRegionMin(); // content boundaries
//    min (roughly (0,0)-Scroll), in window coordinates
IMGUI_FUNCTION(GetWindowContentRegionMin)
CALL_FUNCTION(GetWindowContentRegionMin, ImVec2)
PUSH_NUMBER(ret.x)
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetWindowContentRegionMax(); // content boundaries
//    max (roughly (0,0)+Size-Scroll) where Size can be override with
...
//    "default/current item" visible, consider using SetItemDefaultFocus()
//    instead.
...
END_IMGUI_FUNC
//    IMGUI_API void          SetScrollFromPosX(float local_x, float
//    center_x_ratio = 0.5f);  // adjust scrolling amount to make given position
//    visible. Generally GetCursorStartPos() + offset to compute a valid
//    position.
...
END_IMGUI_FUNC
//    IMGUI_API void          SetScrollFromPosY(float local_y, float
//    center_y_ratio = 0.5f);  // adjust scrolling amount to make given position
//    visible. Generally GetCursorStartPos() + offset to compute a valid
//    position.
...
END_IMGUI_FUNC
//    IMGUI_API void          PushFont(ImFont* font); // use NULL as a shortcut
//    to push default font
...
//    modify a style float variable. always use this if you modify the style
//    after NewFrame().
...
//    val);             // modify a style ImVec2 variable. always use this if
...
END_IMGUI_FUNC
//    IMGUI_API void          PushButtonRepeat(bool repeat); // in 'repeat'
//    mode, Button*() functions return repeated true in a typematic manner
//    (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call
//    IsItemActive() after any Button() to tell if the button is held in the
//    current frame.
...
END_IMGUI_FUNC
//    IMGUI_API void          PushItemWidth(float item_width); // push width of
//    items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f
//    align xx pixels to the right of window (so -FLT_MIN always align width to
//    the right side). 0.0f = default to ~2/3 of windows width,
...
END_IMGUI_FUNC
//    IMGUI_API void          SetNextItemWidth(float item_width); // set width
//    of the _next_ common large "item+label" widget. >0.0f: width in pixels,
//    <0.0f align xx pixels to the right of window (so -FLT_MIN always align
...
//    settings and current cursor position. NOT necessarily the width of last
...
//    push word-wrapping position for Text*() commands. < 0.0f: no wrapping;
...
//    IMGUI_API float         GetFontSize(); // get current font size (= height
//    in pixels) of current font with current scale applied
IMGUI_FUNCTION(GetFontSize)
CALL_FUNCTION(GetFontSize, float)
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetFontTexUvWhitePixel(); // get UV coordinate for
//    a while pixel, useful to draw custom shapes via the ImDrawList API
...
//    color with style alpha applied, packed as a 32-bit value suitable for
//    ImDrawList
...
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API ImU32         GetColorU32(ImU32 col); // retrieve given color
//    with style alpha applied, packed as a 32-bit value suitable for ImDrawList
...
//    color as stored in ImGuiStyle structure. use to feed back into
//    PushStyleColor(), otherwise use GetColorU32() to get style color with
//    style alpha baked in.
// Unsupported return type const
//    IMGUI_API void          Separator(); // separator, generally horizontal.
//    inside a menu bar or in horizontal layout mode, this becomes a vertical
//    separator.
IMGUI_FUNCTION(Separator)
CALL_FUNCTION_NO_RET(Separator)
END_IMGUI_FUNC
//    IMGUI_API void          SameLine(float offset_from_start_x=0.0f, float
//    spacing=-1.0f);  // call between widgets or groups to layout them
...
END_IMGUI_FUNC
//    IMGUI_API void          NewLine(); // undo a SameLine() or force a new
//    line when in an horizontal-layout context.
...
END_IMGUI_FUNC
//    IMGUI_API void          Dummy(const ImVec2& size); // add a dummy item of
//    given size. unlike InvisibleButton(), Dummy() won't take the mouse click
//    or be navigable into.
IMGUI_FUNCTION(Dummy)
IM_VEC_2_ARG(size)
CALL_FUNCTION_NO_RET(Dummy, size)
END_IMGUI_FUNC
//    IMGUI_API void          Indent(float indent_w = 0.0f); // move content
//    position toward the right, by indent_w, or style.IndentSpacing if indent_w
//    <= 0
...
//    position back to the left, by indent_w, or style.IndentSpacing if indent_w
//    <= 0
...
//    + capture the whole group bounding box into one "item" (so you can use
//    IsItemHovered() or layout primitives such as SameLine() on whole group,
//    etc.)
...
//    window-relative coordinates, such as: GetCursorPos, GetCursorStartPos,
...
//    using the main, absolute coordinate system.
...
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetCursorScreenPos(); // cursor position in
//    absolute screen coordinates [0..io.DisplaySize] (useful to work with
//    ImDrawList API)
...
//    position in absolute screen coordinates [0..io.DisplaySize]
IMGUI_FUNCTION(SetCursorScreenPos)
IM_VEC_2_ARG(pos)
CALL_FUNCTION_NO_RET(SetCursorScreenPos, pos)
END_IMGUI_FUNC
//    IMGUI_API void          AlignTextToFramePadding(); // vertically align
//    upcoming text baseline to FramePadding.y so that it will align properly to
//    regularly framed items (call if you have text on a line before a framed
//    item)
...
//    style.ItemSpacing.y (distance in pixels between 2 consecutive lines of
//    text)
...
//    style.FramePadding.y * 2 + style.ItemSpacing.y (distance in pixels between
...
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API void          PushID(const char* str_id); // push string into
//    the ID stack (will hash string).
IMGUI_FUNCTION(PushID)
LABEL_ARG(str_id)
CALL_FUNCTION_NO_RET(PushID, str_id)
END_IMGUI_FUNC
//    IMGUI_API void          PushID(const char* str_id_begin, const char*
...
END_IMGUI_FUNC
//    IMGUI_API void          PushID(const void* ptr_id); // push pointer into
//    the ID stack (will hash pointer).
// Unsupported arg type const void* ptr_id
//    IMGUI_API void          PushID(int int_id); // push integer into the ID
//    stack (will hash integer).
...
END_IMGUI_FUNC
//    IMGUI_API ImGuiID       GetID(const char* str_id); // calculate unique ID
//    (hash of whole ID stack + given parameter). e.g. if you want to query into
...
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API ImGuiID       GetID(const char* str_id_begin, const char*
//    str_id_end);
...
// Unsupported arg type const void* ptr_id
//    IMGUI_API void          TextUnformatted(const char* text, const char*
//    text_end = NULL); // raw text without formatting. Roughly equivalent to
//    Text("%s", text) but: A) doesn't require null terminated string if
//    'text_end' is specified, B) it's faster, no memory copy is done, no buffer
...
//    FramePadding=(0,0) to easily embed within text
...
//    size, ImGuiButtonFlags flags = 0); // flexible button behavior without the
//    visuals, frequently useful to build custom behaviors using the public api
...
//    const ImVec4& tint_col = ImVec4 1 1 1 1, const ImVec4& border_col = ImVec4
//    0 0 0 0);
// Unsupported arg type  const ImVec2& uv0 = ImVec2 0  0
//    IMGUI_API bool          ImageButton(ImTextureID user_texture_id, const
//    ImVec2& size, const ImVec2& uv0 = ImVec2 0  0,  const ImVec2& uv1 = ImVec2
//    1 1, int frame_padding = -1, const ImVec4& bg_col = ImVec4 0 0 0 0, const
//    ImVec4& tint_col = ImVec4 1 1 1 1);    // <0 frame_padding uses default
//    frame padding settings. 0 for no padding
// Unsupported arg type  const ImVec2& uv0 = ImVec2 0  0
...
END_BOOL_POINTER(v)
END_IMGUI_FUNC
//    IMGUI_API bool          CheckboxFlags(const char* label, int* flags, int
//    flags_value);
...
END_IMGUI_FUNC
//    IMGUI_API bool          CheckboxFlags(const char* label, unsigned int*
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          RadioButton(const char* label, int* v, int
//    v_button);           // shortcut to handle the above pattern when value is
//    an integer
...
//    IMGUI_API void          Bullet(); // draw a small circle + keep the cursor
//    on the same line. advance cursor x position by
//    GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
IMGUI_FUNCTION(Bullet)
CALL_FUNCTION_NO_RET(Bullet)
END_IMGUI_FUNC
//    IMGUI_API bool          BeginCombo(const char* label, const char*
//    preview_value, ImGuiComboFlags flags = 0);
// Unsupported arg type  ImGuiComboFlags flags = 0
//    IMGUI_API void          EndCombo(); // only call EndCombo() if
//    BeginCombo() returns true!
IMGUI_FUNCTION(EndCombo)
CALL_FUNCTION_NO_RET(EndCombo)
POP_END_STACK(5)
END_IMGUI_FUNC
//    IMGUI_API bool          Combo(const char* label, int* current_item, const
//    char* const items[], int items_count, int popup_max_height_in_items = -1);
// Unsupported arg type  const char* const items[]
//    IMGUI_API bool          Combo(const char* label, int* current_item, const
//    char* items_separated_by_zeros, int popup_max_height_in_items = -1); //
...
CALL_FUNCTION(Combo, bool, label, current_item, items_separated_by_zeros,
              popup_max_height_in_items)
PUSH_BOOL(ret)
END_INT_POINTER(current_item)
END_IMGUI_FUNC
//    IMGUI_API bool          Combo(const char* label, int* current_item,
//    bool(*items_getter)(void* data, int idx, const char** out_text), void*
//    data, int items_count, int popup_max_height_in_items = -1);
// Unsupported arg type  bool(*items_getter)(void* data
// Unsupported arg type  const char** out_text)
// Unsupported arg type  void* data
//    IMGUI_API bool          DragFloat(const char* label, float* v, float
//    v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* format
//    = "%.3f", ImGuiSliderFlags flags = 0);     // If v_min >= v_max we have no
//    bound
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragFloat2(const char* label, float v[2], float
//    v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* format
//    = "%.3f", ImGuiSliderFlags flags = 0);
// Unsupported arg type  float v[2]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragFloat3(const char* label, float v[3], float
//    v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* format
//    = "%.3f", ImGuiSliderFlags flags = 0);
// Unsupported arg type  float v[3]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragFloat4(const char* label, float v[4], float
//    v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* format
//    = "%.3f", ImGuiSliderFlags flags = 0);
// Unsupported arg type  float v[4]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragFloatRange2(const char* label, float*
//    v_current_min, float* v_current_max, float v_speed = 1.0f, float v_min =
//    0.0f, float v_max = 0.0f, const char* format = "%.3f", const char*
//    format_max = NULL, ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragInt(const char* label, int* v, float v_speed
//    = 1.0f, int v_min = 0, int v_max = 0, const char* format = "%d",
//    ImGuiSliderFlags flags = 0);  // If v_min >= v_max we have no bound
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragInt2(const char* label, int v[2], float
//    v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* format = "%d",
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[2]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragInt3(const char* label, int v[3], float
//    v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* format = "%d",
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[3]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragInt4(const char* label, int v[4], float
//    v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* format = "%d",
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[4]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragIntRange2(const char* label, int*
//    v_current_min, int* v_current_max, float v_speed = 1.0f, int v_min = 0,
//    int v_max = 0, const char* format = "%d", const char* format_max = NULL,
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragScalar(const char* label, ImGuiDataType
//    data_type, void* p_data, float v_speed, const void* p_min = NULL, const
//    void* p_max = NULL, const char* format = NULL, ImGuiSliderFlags flags =
//    0);
...
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          DragScalarN(const char* label, ImGuiDataType
//    data_type, void* p_data, int components, float v_speed, const void* p_min
//    = NULL, const void* p_max = NULL, const char* format = NULL,
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiDataType data_type
// Unsupported arg type  void* p_data
// Unsupported arg type  const void* p_min = NULL
// Unsupported arg type  const void* p_max = NULL
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderFloat(const char* label, float* v, float
//    v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags =
//    0);     // adjust format to decorate the value with a prefix or a suffix
//    for in-slider labels or unit display.
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderFloat2(const char* label, float v[2], float
//    v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags =
//    0);
// Unsupported arg type  float v[2]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderFloat3(const char* label, float v[3], float
//    v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags =
//    0);
// Unsupported arg type  float v[3]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderFloat4(const char* label, float v[4], float
//    v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags =
//    0);
// Unsupported arg type  float v[4]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderAngle(const char* label, float* v_rad, float
//    v_degrees_min = -360.0f, float v_degrees_max = +360.0f, const char* format
//    = "%.0f deg", ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderInt(const char* label, int* v, int v_min,
//    int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderInt2(const char* label, int v[2], int v_min,
//    int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[2]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderInt3(const char* label, int v[3], int v_min,
//    int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[3]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderInt4(const char* label, int v[4], int v_min,
//    int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
// Unsupported arg type  int v[4]
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderScalar(const char* label, ImGuiDataType
...
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          SliderScalarN(const char* label, ImGuiDataType
...
//    size, float* v, float v_min, float v_max, const char* format = "%.3f",
//    ImGuiSliderFlags flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          VSliderInt(const char* label, const ImVec2& size,
//    int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags
//    flags = 0);
// Unsupported arg type  ImGuiSliderFlags flags = 0
//    IMGUI_API bool          VSliderScalar(const char* label, const ImVec2&
//    size, ImGuiDataType data_type, void* p_data, const void* p_min, const
...
//    buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback =
...
//    size_t buf_size, const ImVec2& size = ImVec2 0  0, ImGuiInputTextFlags
//    flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data =
//    NULL);
// Unsupported arg type  char* buf
// Unsupported arg type  size_t buf_size
// Unsupported arg type  const ImVec2& size = ImVec2 0  0
// Unsupported arg type  ImGuiInputTextCallback callback = NULL
// Unsupported arg type  void* user_data = NULL
//    IMGUI_API bool          InputTextWithHint(const char* label, const char*
//    hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0,
...
//    = 0.0f, float step_fast = 0.0f, const char* format = "%.3f",
...
END_FLOAT_POINTER(v)
END_IMGUI_FUNC
//    IMGUI_API bool          InputFloat2(const char* label, float v[2], const
//    char* format = "%.3f", ImGuiInputTextFlags flags = 0);
// Unsupported arg type  float v[2]
//    IMGUI_API bool          InputFloat3(const char* label, float v[3], const
//    char* format = "%.3f", ImGuiInputTextFlags flags = 0);
// Unsupported arg type  float v[3]
//    IMGUI_API bool          InputFloat4(const char* label, float v[4], const
//    char* format = "%.3f", ImGuiInputTextFlags flags = 0);
// Unsupported arg type  float v[4]
...
//    ImGuiInputTextFlags flags = 0);
// Unsupported arg type  int v[4]
//    IMGUI_API bool          InputDouble(const char* label, double* v, double
//    step = 0.0, double step_fast = 0.0, const char* format = "%.6f",
//    ImGuiInputTextFlags flags = 0);
// Unsupported arg type  double* v
// Unsupported arg type  double step = 0.0
// Unsupported arg type  double step_fast = 0.0
//    IMGUI_API bool          InputScalar(const char* label, ImGuiDataType
//    data_type, void* p_data, const void* p_step = NULL, const void*
//    p_step_fast = NULL, const char* format = NULL, ImGuiInputTextFlags flags =
//    0);
// Unsupported arg type  ImGuiDataType data_type
// Unsupported arg type  void* p_data
// Unsupported arg type  const void* p_step = NULL
// Unsupported arg type  const void* p_step_fast = NULL
//    IMGUI_API bool          InputScalarN(const char* label, ImGuiDataType
//    data_type, void* p_data, int components, const void* p_step = NULL, const
//    void* p_step_fast = NULL, const char* format = NULL, ImGuiInputTextFlags
//    flags = 0);
// Unsupported arg type  ImGuiDataType data_type
// Unsupported arg type  void* p_data
// Unsupported arg type  const void* p_step = NULL
...
//    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4&
//    col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2 0  0); // display
//    a color square/button, hover for details, return true when pressed.
// Unsupported arg type  ImGuiColorEditFlags flags = 0
// Unsupported arg type  ImVec2 size = ImVec2 0  0
//    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags); //
//    initialize current options (generally on application startup) if you want
...
//    displayed string. Read the FAQ about why and how to use ID. to align
...
// Unsupported arg type const void* ptr_id
// Unsupported arg type  va_list args) IM_FMTLIST(2
//    IMGUI_API bool          TreeNodeEx(const char* label, ImGuiTreeNodeFlags
//    flags = 0);
// Unsupported arg type  ImGuiTreeNodeFlags flags = 0
//    IMGUI_API bool          TreeNodeEx(const char* str_id, ImGuiTreeNodeFlags
//    flags, const char* fmt, ...) IM_FMTARGS(3);
// Unsupported arg type  ImGuiTreeNodeFlags flags
// Unsupported arg type  ...) IM_FMTARGS(3
//    IMGUI_API bool          TreeNodeEx(const void* ptr_id, ImGuiTreeNodeFlags
//    flags, const char* fmt, ...) IM_FMTARGS(3);
// Unsupported arg type const void* ptr_id
// Unsupported arg type  ImGuiTreeNodeFlags flags
// Unsupported arg type  ...) IM_FMTARGS(3
//    IMGUI_API bool          TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags
//    flags, const char* fmt, va_list args) IM_FMTLIST(3);
// Unsupported arg type  ImGuiTreeNodeFlags flags
// Unsupported arg type  va_list args) IM_FMTLIST(3
//    IMGUI_API bool          TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags
//    flags, const char* fmt, va_list args) IM_FMTLIST(3);
// Unsupported arg type const void* ptr_id
// Unsupported arg type  ImGuiTreeNodeFlags flags
// Unsupported arg type  va_list args) IM_FMTLIST(3
...
//    distance preceding label when using TreeNode*() or Bullet() == (g.FontSize
...
// Unsupported arg type  ImGuiTreeNodeFlags flags = 0
//    IMGUI_API bool          CollapsingHeader(const char* label, bool*
//    p_visible, ImGuiTreeNodeFlags flags = 0); // when 'p_visible != NULL': if
//    '*p_visible==true' display an additional small close button on upper right
//    of the header which will set the bool to false when clicked, if
...
//    false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2 0  0);
...
//    "bool* p_selected" point to the selection state (read-write), as a
//    convenient helper.
// Unsupported arg type  const ImVec2& size = ImVec2 0  0
//    IMGUI_API bool          ListBox(const char* label, int* current_item,
//    const char* const items[], int items_count, int height_in_items = -1);
// Unsupported arg type  const char* const items[]
//    IMGUI_API bool          ListBox(const char* label, int* current_item, bool
//    (*items_getter)(void* data, int idx, const char** out_text), void* data,
//    int items_count, int height_in_items = -1);
// Unsupported arg type  bool (*items_getter)(void* data
// Unsupported arg type  const char** out_text)
// Unsupported arg type  void* data
//    IMGUI_API bool          ListBoxHeader(const char* label, const ImVec2&
//    size = ImVec2 0  0); // use if you want to reimplement ListBox() will
//    custom data or interactions. if the function return true, you can output
...
//    region. only call ListBoxFooter() if ListBoxHeader() returned true!
...
//    float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size =
//    ImVec2 0  0, int stride = sizeof(float));
...
//    FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2 0  0);
// Unsupported arg type  float(*values_getter)(void* data
// Unsupported arg type  void* data
// Unsupported arg type  ImVec2 graph_size = ImVec2 0  0
//    IMGUI_API void          PlotHistogram(const char* label, const float*
//    values, int values_count, int values_offset = 0, const char* overlay_text
//    = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2
//    graph_size = ImVec2 0  0, int stride = sizeof(float));
// Unsupported arg type  const float* values
// Unsupported arg type  ImVec2 graph_size = ImVec2 0  0
...
CALL_FUNCTION_NO_RET(Value, prefix, v)
END_IMGUI_FUNC
//    IMGUI_API void          Value(const char* prefix, float v, const char*
//    float_format = NULL);
...
END_IMGUI_FUNC
//    IMGUI_API bool          BeginMenuBar(); // append to menu-bar of current
...
//    NULL, bool selected = false, bool enabled = true);  // return true when
//    activated. shortcuts are displayed for convenience but not processed by
//    ImGui at the moment
...
END_IMGUI_FUNC
//    IMGUI_API bool          MenuItem(const char* label, const char* shortcut,
//    bool* p_selected, bool enabled = true);              // return true when
//    activated + toggle (*p_selected) if p_selected != NULL
...
END_IMGUI_FUNC
//    IMGUI_API void          BeginTooltip(); // begin/append a tooltip window.
...
//    IM_FMTLIST(1);
// Unsupported arg type  va_list args) IM_FMTLIST(1
//    IMGUI_API bool          BeginPopup(const char* str_id, ImGuiWindowFlags
//    flags = 0);                         // return true if the popup is open,
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          BeginPopupModal(const char* name, bool* p_open =
//    NULL, ImGuiWindowFlags flags = 0); // return true if the modal is open,
//    and you can start outputting to it.
...
POP_END_STACK(11)
END_IMGUI_FUNC
//    IMGUI_API void          OpenPopup(const char* str_id, ImGuiPopupFlags
//    popup_flags = 0);                     // call to mark popup as open (don't
//    call every frame!).
// Unsupported arg type  ImGuiPopupFlags popup_flags = 0
//    IMGUI_API void          OpenPopupOnItemClick(const char* str_id = NULL,
//    ImGuiPopupFlags popup_flags = 1);   // helper to open popup when clicked
//    on last item. return true when just opened. (note: actually triggers on
//    the mouse _released_ event to be consistent with popup behaviors)
// Unsupported arg type  ImGuiPopupFlags popup_flags = 1
//    IMGUI_API void          CloseCurrentPopup(); // manually close the popup
//    we have begin-ed into.
...
//    last item. if you can pass a NULL str_id only if the previous item had an
//    id. If you want to use that on a non-interactive item such as Text() you
...
// Unsupported arg type  ImGuiPopupFlags popup_flags = 1
//    IMGUI_API bool          IsPopupOpen(const char* str_id, ImGuiPopupFlags
//    flags = 0);                         // return true if the popup is open.
...
//    ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImU32
//    user_id = 0);
...
END_IMGUI_FUNC
//    IMGUI_API ImGuiTableSortSpecs* TableGetSortSpecs(); // get latest sort
//    specs for the table (NULL if not sorting).
// Unsupported return type ImGuiTableSortSpecs*
//    IMGUI_API int                   TableGetColumnCount(); // return number of
//    columns (value passed to BeginTable)
// Unsupported return type int
//    IMGUI_API int                   TableGetColumnIndex(); // return current
//    column index.
// Unsupported return type int
//    IMGUI_API int                   TableGetRowIndex(); // return current row
//    index.
// Unsupported return type int
...
//    ImU32 color, int column_n = -1);  // change the color of a cell, row, or
//    column. See ImGuiTableBgTarget_ flags for details.
// Unsupported arg type ImGuiTableBgTarget target
//    IMGUI_API void          Columns(int count = 1, const char* id = NULL, bool
//    border = true);
...
END_IMGUI_FUNC
//    IMGUI_API void          NextColumn(); // next column, defaults to current
//    row or next row if the current row is finished
...
//    position of column line (in pixels, from the left side of the contents
...
//    flags = 0);        // create and append into a TabBar
// Unsupported arg type  ImGuiTabBarFlags flags = 0
//    IMGUI_API void          EndTabBar(); // only call EndTabBar() if
//    BeginTabBar() returns true!
IMGUI_FUNCTION(EndTabBar)
CALL_FUNCTION_NO_RET(EndTabBar)
POP_END_STACK(13)
END_IMGUI_FUNC
//    IMGUI_API bool          BeginTabItem(const char* label, bool* p_open =
//    NULL, ImGuiTabItemFlags flags = 0); // create a Tab. Returns true if the
//    Tab is selected.
...
POP_END_STACK(14)
END_IMGUI_FUNC
//    IMGUI_API bool          TabItemButton(const char* label, ImGuiTabItemFlags
//    flags = 0);      // create a Tab behaving like a button. return true when
...
//    of a closed tab/window ahead (useful to reduce visual flicker on
//    reorderable tab bars). For tab-bar: call after BeginTabBar() and before
...
//    logging to tty (stdout)
IMGUI_FUNCTION(LogToTTY)
OPTIONAL_INT_ARG(auto_open_depth, -1)
CALL_FUNCTION_NO_RET(LogToTTY, auto_open_depth)
END_IMGUI_FUNC
//    IMGUI_API void          LogToFile(int auto_open_depth = -1, const char*
//    filename = NULL);   // start logging to file
...
END_IMGUI_FUNC
//    IMGUI_API void          LogButtons(); // helper to display buttons for
//    logging to tty/file/clipboard
...
//    // call when the current item is active. If this return true, you can call
//    SetDragDropPayload() + EndDragDropSource()
// Unsupported arg type ImGuiDragDropFlags flags = 0
//    IMGUI_API bool          SetDragDropPayload(const char* type, const void*
//    data, size_t sz, ImGuiCond cond = 0);  // type is a user defined string of
//    maximum 32 characters. Strings starting with '_' are reserved for dear
//    imgui internal types. Data is copied and held by imgui.
// Unsupported arg type  const void* data
// Unsupported arg type  size_t sz
// Unsupported arg type  ImGuiCond cond = 0
//    IMGUI_API void          EndDragDropSource(); // only call
//    EndDragDropSource() if BeginDragDropSource() returns true!
IMGUI_FUNCTION(EndDragDropSource)
CALL_FUNCTION_NO_RET(EndDragDropSource)
POP_END_STACK(15)
END_IMGUI_FUNC
//    IMGUI_API bool                  BeginDragDropTarget(); // call after
//    submitting an item that may receive a payload. If this returns true, you
...
//    ImGuiDragDropFlags flags = 0);          // accept contents of a given
//    type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into
//    the payload before the mouse button is released.
// Unsupported return type const
// Unsupported arg type  ImGuiDragDropFlags flags = 0
//    IMGUI_API void                  EndDragDropTarget(); // only call
//    EndDragDropTarget() if BeginDragDropTarget() returns true!
...
//    into the current payload from anywhere. may return NULL. use
//    ImGuiPayload::IsDataType() to test for the payload type.
// Unsupported return type const
//    IMGUI_API void          PushClipRect(const ImVec2& clip_rect_min, const
//    ImVec2& clip_rect_max, bool intersect_with_current_clip_rect);
IMGUI_FUNCTION(PushClipRect)
IM_VEC_2_ARG(clip_rect_min)
IM_VEC_2_ARG(clip_rect_max)
BOOL_ARG(intersect_with_current_clip_rect)
CALL_FUNCTION_NO_RET(PushClipRect, clip_rect_min, clip_rect_max,
...
END_IMGUI_FUNC
//    IMGUI_API void          SetItemDefaultFocus(); // make last item the
//    default focused item of a window.
...
//    keyboard on the next widget. Use positive 'offset' to access sub
...
//    the last item hovered? (and usable, aka not blocked by a popup, etc.). See
//    ImGuiHoveredFlags for more options.
// Unsupported arg type ImGuiHoveredFlags flags = 0
//    IMGUI_API bool          IsItemActive(); // is the last item active? (e.g.
//    button being held, text field being edited. This will continuously return
//    true while holding mouse button on an item. Items that don't interact will
//    always return false)
...
//    IMGUI_API bool          IsItemVisible(); // is the last item visible?
//    (items may be out of sight because of clipping/scrolling)
IMGUI_FUNCTION(IsItemVisible)
CALL_FUNCTION(IsItemVisible, bool)
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsItemEdited(); // did the last item modify its
//    underlying value this frame? or was pressed? This is generally the same as
...
//    active (item was previously inactive).
IMGUI_FUNCTION(IsItemActivated)
CALL_FUNCTION(IsItemActivated, bool)
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsItemDeactivated(); // was the last item just
//    made inactive (item was previously active). Useful for Undo/Redo patterns
//    with widgets that requires continuous editing.
IMGUI_FUNCTION(IsItemDeactivated)
CALL_FUNCTION(IsItemDeactivated, bool)
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsItemDeactivatedAfterEdit(); // was the last item
//    just made inactive and made a value change when it was active? (e.g.
//    Slider/Drag moved). Useful for Undo/Redo patterns with widgets that
//    requires continuous editing. Note that you may get false positives (some
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsItemToggledOpen(); // was the last item open
//    state toggled? set by TreeNode().
...
//    overlapped by a subsequent item. sometimes useful with invisible buttons,
//    selectables, etc. to catch unused area.
IMGUI_FUNCTION(SetItemAllowOverlap)
CALL_FUNCTION_NO_RET(SetItemAllowOverlap)
END_IMGUI_FUNC
//    IMGUI_API bool          IsRectVisible(const ImVec2& size); // test if
//    rectangle (of given size, starting from cursor position) is visible / not
//    clipped.
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const
//    ImVec2& rect_max);      // test if rectangle (in screen space) is visible
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API double        GetTime(); // get global imgui time. incremented
//    by io.DeltaTime every frame.
// Unsupported return type double
//    IMGUI_API int           GetFrameCount(); // get global imgui frame count.
//    incremented by 1 every frame.
// Unsupported return type int
//    IMGUI_API ImDrawList*   GetBackgroundDrawList(); // this draw list will be
//    the first rendering one. Useful to quickly draw shapes/text behind dear
//    imgui contents.
// Unsupported return type ImDrawList*
//    IMGUI_API ImDrawList*   GetForegroundDrawList(); // this draw list will be
//    the last rendered one. Useful to quickly draw shapes/text over dear imgui
//    contents.
// Unsupported return type ImDrawList*
//    IMGUI_API ImDrawListSharedData* GetDrawListSharedData(); // you may use
//    this when creating your own ImDrawList instances.
// Unsupported return type ImDrawListSharedData*
//    IMGUI_API const char*   GetStyleColorName(ImGuiCol idx); // get a string
//    corresponding to the enum value (for display, saving, etc.).
...
//    current window storage with our own (if you want to manipulate it
...
//    IMGUI_API void          CalcListClipping(int items_count, float
//    items_height, int* out_items_display_start, int* out_items_display_end);
//    // calculate coarse clipping for large list of evenly sized items. Prefer
...
CALL_FUNCTION_NO_RET(CalcListClipping, items_count, items_height,
...
//    regardless of BeginChildFrame() return values (which indicates a
//    collapsed/clipped window)
IMGUI_FUNCTION(EndChildFrame)
CALL_FUNCTION_NO_RET(EndChildFrame)
POP_END_STACK(17)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        CalcTextSize(const char* text, const char*
//    text_end = NULL, bool hide_text_after_double_hash = false, float
//    wrap_width = -1.0f);
...
CALL_FUNCTION(CalcTextSize, ImVec2, text, text_end, hide_text_after_double_hash,
              wrap_width)
PUSH_NUMBER(ret.x)
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
...
PUSH_NUMBER(ret)
END_IMGUI_FUNC
//    IMGUI_API void          ColorConvertRGBtoHSV(float r, float g, float b,
...
//    IMGUI_API void          ColorConvertHSVtoRGB(float h, float s, float v,
...
//    values into user's key index. == io.KeyMap[key]
// Unsupported return type int
//    IMGUI_API bool          IsKeyDown(int user_key_index); // is key being
//    held. == io.KeysDown[user_key_index].
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          IsKeyReleased(int user_key_index); // was key
//    released (went from Down to !Down)?
IMGUI_FUNCTION(IsKeyReleased)
INT_ARG(user_key_index)
CALL_FUNCTION(IsKeyReleased, bool, user_key_index)
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API int           GetKeyPressedAmount(int key_index, float
//    repeat_delay, float rate); // uses provided repeat rate/delay. return a
//    count, most often 0 or 1 but might be >1 if RepeatRate is small enough
//    that DeltaTime > RepeatRate
// Unsupported return type int
//    IMGUI_API void          CaptureKeyboardFromApp(bool
//    want_capture_keyboard_value = true);    // attention: misleading name!
//    manually override io.WantCaptureKeyboard flag next frame (said flag is
//    entirely left for your application to handle). e.g. force capture keyboard
//    when your widget is being hovered. This is equivalent to setting
//    "io.WantCaptureKeyboard = want_capture_keyboard_value"; after the next
//    NewFrame() call.
...
//    IMGUI_API bool          IsMouseClicked(ImGuiMouseButton button, bool
//    repeat = false);       // did mouse button clicked? (went from !Down to
//    Down)
// Unsupported arg type ImGuiMouseButton button
//    IMGUI_API bool          IsMouseReleased(ImGuiMouseButton button); // did
//    mouse button released? (went from Down to !Down)
// Unsupported arg type ImGuiMouseButton button
//    IMGUI_API bool          IsMouseDoubleClicked(ImGuiMouseButton button); //
//    did mouse button double-clicked? (note that a double-click will also
//    report IsMouseClicked() == true)
// Unsupported arg type ImGuiMouseButton button
//    IMGUI_API bool          IsMouseHoveringRect(const ImVec2& r_min, const
//    ImVec2& r_max, bool clip = true);// is mouse hovering given bounding rect
//    (in screen space). clipped by current clipping settings, but disregarding
...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API ImVec2        GetMousePos(); // shortcut to
//    ImGui::GetIO().MousePos provided by user, to be consistent with other
//    calls
...
//    mouse position at the time of opening popup we have BeginPopup() into
...
PUSH_NUMBER(ret.y)
END_IMGUI_FUNC
//    IMGUI_API bool          IsMouseDragging(ImGuiMouseButton button, float
//    lock_threshold = -1.0f);         // is mouse dragging? (if lock_threshold
...
//    clicking position while the mouse button is pressed or was just released.
...
//    reset in ImGui::NewFrame(), this is updated during the frame. valid before
...
//    io.WantCaptureMouse flag next frame (said flag is entirely left for your
//    application to handle). This is equivalent to setting "io.WantCaptureMouse
...
//    seconds after any modification that should be reflected in the .ini file
...
//    which you can save by your own mean. call when io.WantSaveIniSettings is
//    set, then save data by your own mean and clear io.WantSaveIniSettings.
// Unsupported arg type size_t* out_ini_size = NULL
//    IMGUI_API bool          DebugCheckVersionAndDataLayout(const char*
//    version_str, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t
//    sz_vec4, size_t sz_drawvert, size_t sz_drawidx); // This is called by
//    IMGUI_CHECKVERSION() macro.
...
//    IMGUI_API void          SetAllocatorFunctions(void* (*alloc_func)(size_t
//    sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void*
//    user_data = NULL);
...
//    ImGuiWindowFlags_NoResize               = 1 << 1,   // Disable user
//    resizing with the lower-right grip
MAKE_ENUM(ImGuiWindowFlags_NoResize, NoResize)
//    ImGuiWindowFlags_NoMove                 = 1 << 2,   // Disable user moving
//    the window
MAKE_ENUM(ImGuiWindowFlags_NoMove, NoMove)
//    ImGuiWindowFlags_NoScrollbar            = 1 << 3,   // Disable scrollbars
//    (window can still scroll with mouse or programmatically)
MAKE_ENUM(ImGuiWindowFlags_NoScrollbar, NoScrollbar)
//    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 4,   // Disable user
//    vertically scrolling with mouse wheel. On child window, mouse wheel will
//    be forwarded to the parent unless NoScrollbar is also set.
MAKE_ENUM(ImGuiWindowFlags_NoScrollWithMouse, NoScrollWithMouse)
//    ImGuiWindowFlags_NoCollapse             = 1 << 5,   // Disable user
//    collapsing window by double-clicking on it
MAKE_ENUM(ImGuiWindowFlags_NoCollapse, NoCollapse)
//    ImGuiWindowFlags_AlwaysAutoResize       = 1 << 6,   // Resize every window
//    to its content every frame
MAKE_ENUM(ImGuiWindowFlags_AlwaysAutoResize, AlwaysAutoResize)
//    ImGuiWindowFlags_NoBackground           = 1 << 7,   // Disable drawing
//    background color (WindowBg, etc.) and outside border. Similar as using
//    SetNextWindowBgAlpha(0.0f).
MAKE_ENUM(ImGuiWindowFlags_NoBackground, NoBackground)
//    ImGuiWindowFlags_NoSavedSettings        = 1 << 8,   // Never load/save
//    settings in .ini file
MAKE_ENUM(ImGuiWindowFlags_NoSavedSettings, NoSavedSettings)
//    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching
//    mouse, hovering test with pass through.
...
//    scrollbar to appear (off by default). You may use
...
//    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // Disable taking
...
//    window to front when taking focus (e.g. clicking on it or programmatically
//    giving it focus)
...
//    windows without border uses style.WindowPadding (ignored by default for
...
//    ImGuiWindowFlags_NoNavFocus             = 1 << 19,  // No focusing toward
//    this window with gamepad/keyboard navigation (e.g. skipped by CTRL+TAB)
MAKE_ENUM(ImGuiWindowFlags_NoNavFocus, NoNavFocus)
//    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // Append '*' to title
//    without affecting the ID, as a convenience to avoid using the ###
//    operator. When used in a tab/docking context, tab is selected on closure
//    and closure is deferred by one frame to allow code to cancel the closure
...
//    gamepad/keyboard navigation to cross over parent border to this child
//    (only use on child that have no scrolling!)
MAKE_ENUM(ImGuiWindowFlags_NavFlattened, NavFlattened)
//    ImGuiWindowFlags_ChildWindow            = 1 << 24,  // Don't use! For
//    internal use by BeginChild()
MAKE_ENUM(ImGuiWindowFlags_ChildWindow, ChildWindow)
//    ImGuiWindowFlags_Tooltip                = 1 << 25,  // Don't use! For
//    internal use by BeginTooltip()
MAKE_ENUM(ImGuiWindowFlags_Tooltip, Tooltip)
//    ImGuiWindowFlags_Popup                  = 1 << 26,  // Don't use! For
//    internal use by BeginPopup()
MAKE_ENUM(ImGuiWindowFlags_Popup, Popup)
//    ImGuiWindowFlags_Modal                  = 1 << 27,  // Don't use! For
//    internal use by BeginPopupModal()
MAKE_ENUM(ImGuiWindowFlags_Modal, Modal)
//    ImGuiWindowFlags_ChildMenu              = 1 << 28   // Don't use! For
//    internal use by BeginMenu()
...
//    ImGuiInputTextFlags_CharsNoBlank        = 1 << 3,   // Filter out spaces,
//    tabs
MAKE_ENUM(ImGuiInputTextFlags_CharsNoBlank, CharsNoBlank)
//    ImGuiInputTextFlags_AutoSelectAll       = 1 << 4,   // Select entire text
//    when first taking mouse focus
MAKE_ENUM(ImGuiInputTextFlags_AutoSelectAll, AutoSelectAll)
//    ImGuiInputTextFlags_EnterReturnsTrue    = 1 << 5,   // Return 'true' when
//    Enter is pressed (as opposed to every time the value was modified).
...
//    ImGuiInputTextFlags_CallbackCompletion  = 1 << 6,   // Callback on
//    pressing TAB (for completion handling)
MAKE_ENUM(ImGuiInputTextFlags_CallbackCompletion, CallbackCompletion)
//    ImGuiInputTextFlags_CallbackHistory     = 1 << 7,   // Callback on
//    pressing Up/Down arrows (for history handling)
MAKE_ENUM(ImGuiInputTextFlags_CallbackHistory, CallbackHistory)
//    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each
//    iteration. User code may query cursor position, modify text buffer.
MAKE_ENUM(ImGuiInputTextFlags_CallbackAlways, CallbackAlways)
//    ImGuiInputTextFlags_CallbackCharFilter  = 1 << 9,   // Callback on
//    character inputs to replace or discard them. Modify 'EventChar' to replace
//    or discard, or return 1 in callback to discard.
MAKE_ENUM(ImGuiInputTextFlags_CallbackCharFilter, CallbackCharFilter)
//    ImGuiInputTextFlags_AllowTabInput       = 1 << 10,  // Pressing TAB input
//    a '\t' character into the text field
MAKE_ENUM(ImGuiInputTextFlags_AllowTabInput, AllowTabInput)
//    ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 11,  // In multi-line mode,
//    unfocus with Enter, add new line with Ctrl+Enter (default is opposite:
...
//    ImGuiInputTextFlags_NoUndoRedo          = 1 << 16,  // Disable undo/redo.
//    Note that input text owns the text data while active, if you want to
...
//    0123456789.+-*/eE (Scientific notation input)
MAKE_ENUM(ImGuiInputTextFlags_CharsScientific, CharsScientific)
//    ImGuiInputTextFlags_CallbackResize      = 1 << 18,  // Callback on buffer
//    capacity changes request (beyond 'buf_size' parameter value), allowing the
//    string to grow. Notify when the string wants to be resized (for string
//    types which hold a cache of their Size). You will be provided a new
//    BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h
//    for an example of using this)
MAKE_ENUM(ImGuiInputTextFlags_CallbackResize, CallbackResize)
//    ImGuiInputTextFlags_CallbackEdit        = 1 << 19,  // Callback on any
//    edit (note that InputText() already returns true on edit, the callback is
...
//    functions using InputText() before reformatting data
...
//    ImGuiTreeNodeFlags_Framed               = 1 << 1,   // Draw frame with
//    background (e.g. for CollapsingHeader)
MAKE_ENUM(ImGuiTreeNodeFlags_Framed, Framed)
//    ImGuiTreeNodeFlags_AllowItemOverlap     = 1 << 2,   // Hit testing to
//    allow subsequent widgets to overlap this one
MAKE_ENUM(ImGuiTreeNodeFlags_AllowItemOverlap, AllowItemOverlap)
//    ImGuiTreeNodeFlags_NoTreePushOnOpen     = 1 << 3,   // Don't do a
//    TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor
//    pushing on ID stack
MAKE_ENUM(ImGuiTreeNodeFlags_NoTreePushOnOpen, NoTreePushOnOpen)
//    ImGuiTreeNodeFlags_NoAutoOpenOnLog      = 1 << 4,   // Don't automatically
//    and temporarily open node when Logging is active (by default logging will
//    automatically open tree nodes)
MAKE_ENUM(ImGuiTreeNodeFlags_NoAutoOpenOnLog, NoAutoOpenOnLog)
//    ImGuiTreeNodeFlags_DefaultOpen          = 1 << 5,   // Default node to be
//    open
MAKE_ENUM(ImGuiTreeNodeFlags_DefaultOpen, DefaultOpen)
//    ImGuiTreeNodeFlags_OpenOnDoubleClick    = 1 << 6,   // Need double-click
//    to open node
MAKE_ENUM(ImGuiTreeNodeFlags_OpenOnDoubleClick, OpenOnDoubleClick)
//    ImGuiTreeNodeFlags_OpenOnArrow          = 1 << 7,   // Only open when
//    clicking on the arrow part. If ImGuiTreeNodeFlags_OpenOnDoubleClick is
//    also set, single-click arrow or double-click all box to open.
MAKE_ENUM(ImGuiTreeNodeFlags_OpenOnArrow, OpenOnArrow)
//    ImGuiTreeNodeFlags_Leaf                 = 1 << 8,   // No collapsing, no
//    arrow (use as a convenience for leaf nodes).
MAKE_ENUM(ImGuiTreeNodeFlags_Leaf, Leaf)
//    ImGuiTreeNodeFlags_Bullet               = 1 << 9,   // Display a bullet
//    instead of arrow
MAKE_ENUM(ImGuiTreeNodeFlags_Bullet, Bullet)
//    ImGuiTreeNodeFlags_FramePadding         = 1 << 10,  // Use FramePadding
//    (even for an unframed text node) to vertically align text baseline to
...
//    ImGuiTreeNodeFlags_SpanAvailWidth       = 1 << 11,  // Extend hit box to
//    the right-most edge, even if not framed. This is not the default in order
//    to allow adding other items on the same line. In the future we may
//    refactor the hit system to be front-to-back, allowing natural overlaps and
//    then this can become the default.
MAKE_ENUM(ImGuiTreeNodeFlags_SpanAvailWidth, SpanAvailWidth)
//    ImGuiTreeNodeFlags_SpanFullWidth        = 1 << 12,  // Extend hit box to
//    the left-most and right-most edges (bypass the indented area).
MAKE_ENUM(ImGuiTreeNodeFlags_SpanFullWidth, SpanFullWidth)
//    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 13,  // (WIP) Nav: left
//    direction may move to this TreeNode() from any of its child (items
...
//    BeginPopupContext*(): open on Left Mouse release. Guaranteed to always be
...
//    BeginPopupContext*(): open on Right Mouse release. Guaranteed to always be
...
//    BeginPopupContext*(): open on Middle Mouse release. Guaranteed to always
...
//    BeginPopupContext*(): don't open if there's already a popup at the same
...
//    BeginPopupContextWindow(): don't return true when hovering items, only
//    when hovering empty space
MAKE_ENUM(ImGuiPopupFlags_NoOpenOverItems, NoOpenOverItems)
//    ImGuiPopupFlags_AnyPopupId              = 1 << 7,   // For IsPopupOpen():
...
//    ImGuiPopupFlags_AnyPopup                = ImGuiPopupFlags_AnyPopupId |
...
//    ImGuiSelectableFlags_SpanAllColumns     = 1 << 1,   // Selectable frame
//    can span all columns (text will still fit in current column)
MAKE_ENUM(ImGuiSelectableFlags_SpanAllColumns, SpanAllColumns)
//    ImGuiSelectableFlags_AllowDoubleClick   = 1 << 2,   // Generate press
//    events on double clicks too
MAKE_ENUM(ImGuiSelectableFlags_AllowDoubleClick, AllowDoubleClick)
//    ImGuiSelectableFlags_Disabled           = 1 << 3,   // Cannot be selected,
//    display grayed out text
MAKE_ENUM(ImGuiSelectableFlags_Disabled, Disabled)
//    ImGuiSelectableFlags_AllowItemOverlap   = 1 << 4    // (WIP) Hit testing
//    to allow subsequent widgets to overlap this one
...
//    ImGuiComboFlags_PopupAlignLeft          = 1 << 0,   // Align the popup
//    toward the left by default
MAKE_ENUM(ImGuiComboFlags_PopupAlignLeft, PopupAlignLeft)
//    ImGuiComboFlags_HeightSmall             = 1 << 1,   // Max ~4 items
//    visible. Tip: If you want your combo popup to be a specific size you can
//    use SetNextWindowSizeConstraints() prior to calling BeginCombo()
MAKE_ENUM(ImGuiComboFlags_HeightSmall, HeightSmall)
//    ImGuiComboFlags_HeightRegular           = 1 << 2,   // Max ~8 items
//    visible (default)
MAKE_ENUM(ImGuiComboFlags_HeightRegular, HeightRegular)
//    ImGuiComboFlags_HeightLarge             = 1 << 3,   // Max ~20 items
//    visible
MAKE_ENUM(ImGuiComboFlags_HeightLarge, HeightLarge)
//    ImGuiComboFlags_HeightLargest           = 1 << 4,   // As many fitting
//    items as possible
MAKE_ENUM(ImGuiComboFlags_HeightLargest, HeightLargest)
//    ImGuiComboFlags_NoArrowButton           = 1 << 5,   // Display on the
//    preview box without the square arrow button
MAKE_ENUM(ImGuiComboFlags_NoArrowButton, NoArrowButton)
//    ImGuiComboFlags_NoPreview               = 1 << 6,   // Display only a
//    square arrow button
...
//    manually dragging tabs to re-order them + New tabs are appended at the end
//    of list
...
//    buttons to open the tab list popup
...
//    behavior of closing tabs (that are submitted with p_open != NULL) with
...
//    scrolling buttons (apply when fitting policy is
...
//    tooltips when hovering a tab
MAKE_ENUM(ImGuiTabBarFlags_NoTooltip, NoTooltip)
//    ImGuiTabBarFlags_FittingPolicyResizeDown        = 1 << 6,   // Resize tabs
//    when they don't fit
MAKE_ENUM(ImGuiTabBarFlags_FittingPolicyResizeDown, FittingPolicyResizeDown)
//    ImGuiTabBarFlags_FittingPolicyScroll            = 1 << 7,   // Add scroll
//    buttons when tabs don't fit
...
//    ImGuiTabItemFlags_UnsavedDocument               = 1 << 0,   // Append '*'
//    to title without affecting the ID, as a convenience to avoid using the ###
...
//    flag to programmatically make the tab selected when calling BeginTabItem()
...
//    PushID(tab->ID)/PopID() on BeginTabItem()/EndTabItem()
MAKE_ENUM(ImGuiTabItemFlags_NoPushId, NoPushId)
//    ImGuiTabItemFlags_NoTooltip                     = 1 << 4,   // Disable
//    tooltip for the given tab
MAKE_ENUM(ImGuiTabItemFlags_NoTooltip, NoTooltip)
//    ImGuiTabItemFlags_NoReorder                     = 1 << 5,   // Disable
//    reordering this tab or having another tab cross over this tab
MAKE_ENUM(ImGuiTabItemFlags_NoReorder, NoReorder)
//    ImGuiTabItemFlags_Leading                       = 1 << 6,   // Enforce the
//    tab position to the left of the tab bar (after the tab list popup button)
MAKE_ENUM(ImGuiTabItemFlags_Leading, Leading)
//    ImGuiTabItemFlags_Trailing                      = 1 << 7    // Enforce the
//    tab position to the right of the tab bar (before the scrolling buttons)
...
//    persisting columns order, width and sort settings in the .ini file.
MAKE_ENUM(ImGuiTableFlags_NoSavedSettings, NoSavedSettings)
//    ImGuiTableFlags_ContextMenuInBody          = 1 << 5,   // Right-click on
//    columns body/contents will display table context menu. By default it is
//    available in TableHeadersRow().
MAKE_ENUM(ImGuiTableFlags_ContextMenuInBody, ContextMenuInBody)
//    ImGuiTableFlags_RowBg                      = 1 << 6,   // Set each RowBg
//    color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent of
//    calling TableSetBgColor with ImGuiTableBgFlags_RowBg0 on each row
//    manually)
MAKE_ENUM(ImGuiTableFlags_RowBg, RowBg)
//    ImGuiTableFlags_BordersInnerH              = 1 << 7,   // Draw horizontal
//    borders between rows.
MAKE_ENUM(ImGuiTableFlags_BordersInnerH, BordersInnerH)
//    ImGuiTableFlags_BordersOuterH              = 1 << 8,   // Draw horizontal
//    borders at the top and bottom.
MAKE_ENUM(ImGuiTableFlags_BordersOuterH, BordersOuterH)
//    ImGuiTableFlags_BordersInnerV              = 1 << 9,   // Draw vertical
//    borders between columns.
MAKE_ENUM(ImGuiTableFlags_BordersInnerV, BordersInnerV)
//    ImGuiTableFlags_BordersOuterV              = 1 << 10,  // Draw vertical
...
//    ImGuiTableFlags_NoBordersInBody            = 1 << 11,  // [ALPHA] Disable
//    vertical borders in columns Body (borders will always appears in Headers).
//    -> May move to style
MAKE_ENUM(ImGuiTableFlags_NoBordersInBody, NoBordersInBody)
//    ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12,  // [ALPHA] Disable
//    vertical borders in columns Body until hovered for resize (borders will
//    always appears in Headers). -> May move to style
MAKE_ENUM(ImGuiTableFlags_NoBordersInBodyUntilResize,
          NoBordersInBodyUntilResize)
//    ImGuiTableFlags_SizingFixedFit             = 1 << 13,  // Columns default
//    to _WidthFixed or _WidthAuto (if resizable or not resizable), matching
//    contents width.
MAKE_ENUM(ImGuiTableFlags_SizingFixedFit, SizingFixedFit)
//    ImGuiTableFlags_SizingFixedSame            = 2 << 13,  // Columns default
//    to _WidthFixed or _WidthAuto (if resizable or not resizable), matching the
//    maximum contents width of all columns. Implicitly enable
//    ImGuiTableFlags_NoKeepColumnsVisible.
MAKE_ENUM(ImGuiTableFlags_SizingFixedSame, SizingFixedSame)
//    ImGuiTableFlags_SizingStretchProp          = 3 << 13,  // Columns default
//    to _WidthStretch with default weights proportional to each columns
//    contents widths.
MAKE_ENUM(ImGuiTableFlags_SizingStretchProp, SizingStretchProp)
//    ImGuiTableFlags_SizingStretchSame          = 4 << 13,  // Columns default
//    to _WidthStretch with default weights all equal, unless overriden by
//    TableSetupColumn().
MAKE_ENUM(ImGuiTableFlags_SizingStretchSame, SizingStretchSame)
//    ImGuiTableFlags_NoHostExtendX              = 1 << 16,  // Make outer width
//    auto-fit to columns, overriding outer_size.x value. Only available when
//    ScrollX/ScrollY are disabled and Stretch columns are not used.
MAKE_ENUM(ImGuiTableFlags_NoHostExtendX, NoHostExtendX)
//    ImGuiTableFlags_NoHostExtendY              = 1 << 17,  // Make outer
//    height stop exactly at outer_size.y (prevent auto-extending table past the
//    limit). Only available when ScrollX/ScrollY are disabled. Data below the
//    limit will be clipped and not visible.
MAKE_ENUM(ImGuiTableFlags_NoHostExtendY, NoHostExtendY)
//    ImGuiTableFlags_NoKeepColumnsVisible       = 1 << 18,  // Disable keeping
//    column always minimally visible when ScrollX is off and table gets too
...
//    distributing remainder width to stretched columns (width allocation on a
...
//    ImGuiTableFlags_NoClip                     = 1 << 20,  // Disable clipping
//    rectangle for every individual columns (reduce draw command count, items
//    will be able to overflow into other columns). Generally incompatible with
//    TableSetupScrollFreeze().
MAKE_ENUM(ImGuiTableFlags_NoClip, NoClip)
//    ImGuiTableFlags_PadOuterX                  = 1 << 21,  // Default if
//    BordersOuterV is on. Enable outer-most padding. Generally desirable if you
//    have headers.
MAKE_ENUM(ImGuiTableFlags_PadOuterX, PadOuterX)
//    ImGuiTableFlags_NoPadOuterX                = 1 << 22,  // Default if
//    BordersOuterV is off. Disable outer-most padding.
MAKE_ENUM(ImGuiTableFlags_NoPadOuterX, NoPadOuterX)
//    ImGuiTableFlags_NoPadInnerX                = 1 << 23,  // Disable inner
//    padding between columns (double inner padding if BordersOuterV is on,
...
//    horizontal scrolling. Require 'outer_size' parameter of BeginTable() to
//    specify the container size. Changes default sizing policy. Because this
//    create a child window, ScrollY is currently generally recommended when
//    using ScrollX.
MAKE_ENUM(ImGuiTableFlags_ScrollX, ScrollX)
//    ImGuiTableFlags_ScrollY                    = 1 << 25,  // Enable vertical
//    scrolling. Require 'outer_size' parameter of BeginTable() to specify the
//    container size.
MAKE_ENUM(ImGuiTableFlags_ScrollY, ScrollY)
//    ImGuiTableFlags_SortMulti                  = 1 << 26,  // Hold shift when
//    clicking headers to sort on multiple column. TableGetSortSpecs() may
//    return specs where (SpecsCount > 1).
MAKE_ENUM(ImGuiTableFlags_SortMulti, SortMulti)
//    ImGuiTableFlags_SortTristate               = 1 << 27,  // Allow no
//    sorting, disable default sorting. TableGetSortSpecs() may return specs
//    where (SpecsCount == 0).
...
//    ImGuiTableColumnFlags_DefaultHide           = 1 << 0,   // Default as a
//    hidden/disabled column.
MAKE_ENUM(ImGuiTableColumnFlags_DefaultHide, DefaultHide)
//    ImGuiTableColumnFlags_DefaultSort           = 1 << 1,   // Default as a
//    sorting column.
MAKE_ENUM(ImGuiTableColumnFlags_DefaultSort, DefaultSort)
//    ImGuiTableColumnFlags_WidthStretch          = 1 << 2,   // Column will
//    stretch. Preferable with horizontal scrolling disabled (default if table
//    sizing policy is _SizingStretchSame or _SizingStretchProp).
MAKE_ENUM(ImGuiTableColumnFlags_WidthStretch, WidthStretch)
//    ImGuiTableColumnFlags_WidthFixed            = 1 << 3,   // Column will not
//    stretch. Preferable with horizontal scrolling enabled (default if table
//    sizing policy is _SizingFixedFit and table is resizable).
MAKE_ENUM(ImGuiTableColumnFlags_WidthFixed, WidthFixed)
//    ImGuiTableColumnFlags_NoResize              = 1 << 4,   // Disable manual
//    resizing.
MAKE_ENUM(ImGuiTableColumnFlags_NoResize, NoResize)
//    ImGuiTableColumnFlags_NoReorder             = 1 << 5,   // Disable manual
//    reordering this column, this will also prevent other columns from crossing
//    over this column.
MAKE_ENUM(ImGuiTableColumnFlags_NoReorder, NoReorder)
//    ImGuiTableColumnFlags_NoHide                = 1 << 6,   // Disable ability
//    to hide/disable this column.
MAKE_ENUM(ImGuiTableColumnFlags_NoHide, NoHide)
//    ImGuiTableColumnFlags_NoClip                = 1 << 7,   // Disable
//    clipping for this column (all NoClip columns will render in a same draw
//    command).
MAKE_ENUM(ImGuiTableColumnFlags_NoClip, NoClip)
//    ImGuiTableColumnFlags_NoSort                = 1 << 8,   // Disable ability
//    to sort on this field (even if ImGuiTableFlags_Sortable is set on the
//    table).
MAKE_ENUM(ImGuiTableColumnFlags_NoSort, NoSort)
//    ImGuiTableColumnFlags_NoSortAscending       = 1 << 9,   // Disable ability
//    to sort in the ascending direction.
MAKE_ENUM(ImGuiTableColumnFlags_NoSortAscending, NoSortAscending)
//    ImGuiTableColumnFlags_NoSortDescending      = 1 << 10,  // Disable ability
//    to sort in the descending direction.
MAKE_ENUM(ImGuiTableColumnFlags_NoSortDescending, NoSortDescending)
//    ImGuiTableColumnFlags_NoHeaderWidth         = 1 << 11,  // Disable header
//    text width contribution to automatic column width.
MAKE_ENUM(ImGuiTableColumnFlags_NoHeaderWidth, NoHeaderWidth)
//    ImGuiTableColumnFlags_PreferSortAscending   = 1 << 12,  // Make the
//    initial sort direction Ascending when first sorting on this column
//    (default).
MAKE_ENUM(ImGuiTableColumnFlags_PreferSortAscending, PreferSortAscending)
//    ImGuiTableColumnFlags_PreferSortDescending  = 1 << 13,  // Make the
//    initial sort direction Descending when first sorting on this column.
MAKE_ENUM(ImGuiTableColumnFlags_PreferSortDescending, PreferSortDescending)
//    ImGuiTableColumnFlags_IndentEnable          = 1 << 14,  // Use current
//    Indent value when entering cell (default for column 0).
MAKE_ENUM(ImGuiTableColumnFlags_IndentEnable, IndentEnable)
//    ImGuiTableColumnFlags_IndentDisable         = 1 << 15,  // Ignore current
//    Indent value when entering cell (default for columns > 0). Indentation
//    changes _within_ the cell will still be honored.
MAKE_ENUM(ImGuiTableColumnFlags_IndentDisable, IndentDisable)
//    ImGuiTableColumnFlags_IsEnabled             = 1 << 20,  // Status: is
//    enabled == not hidden by user/api (referred to as "Hide" in _DefaultHide
//    and _NoHide) flags.
MAKE_ENUM(ImGuiTableColumnFlags_IsEnabled, IsEnabled)
//    ImGuiTableColumnFlags_IsVisible             = 1 << 21,  // Status: is
//    visible == is enabled AND not clipped by scrolling.
MAKE_ENUM(ImGuiTableColumnFlags_IsVisible, IsVisible)
//    ImGuiTableColumnFlags_IsSorted              = 1 << 22,  // Status: is
//    currently part of the sort specs
MAKE_ENUM(ImGuiTableColumnFlags_IsSorted, IsSorted)
//    ImGuiTableColumnFlags_IsHovered             = 1 << 23,  // Status: is
//    hovered by mouse
...
//    header row (set default background color + width of its contents accounted
...
//    ImGuiTableBgTarget_RowBg0                       = 1,        // Set row
//    background color 0 (generally used for background, automatically set when
//    ImGuiTableFlags_RowBg is used)
MAKE_ENUM(ImGuiTableBgTarget_RowBg0, RowBg0)
//    ImGuiTableBgTarget_RowBg1                       = 2,        // Set row
//    background color 1 (generally used for selection marking)
MAKE_ENUM(ImGuiTableBgTarget_RowBg1, RowBg1)
//    ImGuiTableBgTarget_CellBg                       = 3         // Set cell
//    background color (top-most color)
...
//    IsWindowFocused(): Test from root window (top most parent of the current
//    hierarchy)
MAKE_ENUM(ImGuiFocusedFlags_RootWindow, RootWindow)
//    ImGuiFocusedFlags_AnyWindow                     = 1 << 2,   //
//    IsWindowFocused(): Return true if any window is focused. Important: If you
...
//    ImGuiHoveredFlags_None                          = 0,        // Return true
//    if directly over the item/window, not obstructed by another window, not
...
//    ImGuiHoveredFlags_AllowWhenBlockedByPopup       = 1 << 3,   // Return true
//    even if a popup window is normally blocking access to this item/window
...
//    even if an active item is blocking access to this item/window. Useful for
//    Drag and Drop patterns.
MAKE_ENUM(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem,
          AllowWhenBlockedByActiveItem)
//    ImGuiHoveredFlags_AllowWhenOverlapped           = 1 << 6,   // Return true
//    even if the position is obstructed or overlapped by another window
MAKE_ENUM(ImGuiHoveredFlags_AllowWhenOverlapped, AllowWhenOverlapped)
//    ImGuiHoveredFlags_AllowWhenDisabled             = 1 << 7,   // Return true
...
//    ImGuiDragDropFlags_SourceNoPreviewTooltip       = 1 << 0,   // By default,
//    a successful call to BeginDragDropSource opens a tooltip so you can
//    display a preview or description of the source contents. This flag disable
//    this behavior.
MAKE_ENUM(ImGuiDragDropFlags_SourceNoPreviewTooltip, SourceNoPreviewTooltip)
//    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default,
//    when dragging we clear data so that IsItemHovered() will return false, to
//    avoid subsequent user code submitting tooltips. This flag disable this
...
//    behavior that allows to open tree nodes and collapsing header by holding
//    over them while dragging a source item.
MAKE_ENUM(ImGuiDragDropFlags_SourceNoHoldToOpenOthers, SourceNoHoldToOpenOthers)
//    ImGuiDragDropFlags_SourceAllowNullID            = 1 << 3,   // Allow items
//    such as Text(), Image() that have no unique identifier to be used as drag
//    source, by manufacturing a temporary identifier based on their
...
//    source (from outside of dear imgui), won't attempt to read current
...
//    released. You can then call IsDelivery() to test if the payload needs to
//    be delivered.
...
//    the default highlight rectangle when hovering over target.
...
//    hiding the BeginDragDropSource tooltip from the BeginDragDropTarget site.
...
//    inspecting the payload before delivery.
...
//    ImGuiNavInput_Menu,          // tap: toggle menu / hold: focus, move,
//    resize // e.g. Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
...
//    navigation enable flag. NewFrame() will automatically fill io.NavInputs[]
//    based on io.KeysDown[].
MAKE_ENUM(ImGuiConfigFlags_NavEnableKeyboard, NavEnableKeyboard)
//    ImGuiConfigFlags_NavEnableGamepad       = 1 << 1,   // Master gamepad
//    navigation enable flag. This is mostly to instruct your imgui backend to
//    fill io.NavInputs[]. Backend also needs to set
//    ImGuiBackendFlags_HasGamepad.
MAKE_ENUM(ImGuiConfigFlags_NavEnableGamepad, NavEnableGamepad)
//    ImGuiConfigFlags_NavEnableSetMousePos   = 1 << 2,   // Instruct navigation
//    to move the mouse cursor. May be useful on TV/console systems where moving
//    a virtual mouse is awkward. Will update io.MousePos and set
//    io.WantSetMousePos=true. If enabled you MUST honor io.WantSetMousePos
//    requests in your backend, otherwise ImGui will react as if the mouse is
...
//    ImGuiConfigFlags_NoMouse                = 1 << 4,   // Instruct imgui to
//    clear mouse position/buttons in NewFrame(). This allows ignoring the mouse
...
//    not alter mouse cursor shape and visibility. Use if the backend cursor
//    changes are interfering with yours and you don't want to use
...
//    ImGuiConfigFlags_IsSRGB                 = 1 << 20,  // Application is
//    SRGB-aware.
MAKE_ENUM(ImGuiConfigFlags_IsSRGB, IsSRGB)
//    ImGuiConfigFlags_IsTouchScreen          = 1 << 21   // Application is
//    using a touch screen instead of a mouse.
...
//    ImGuiBackendFlags_HasGamepad            = 1 << 0,   // Backend Platform
//    supports gamepad and currently has one connected.
MAKE_ENUM(ImGuiBackendFlags_HasGamepad, HasGamepad)
//    ImGuiBackendFlags_HasMouseCursors       = 1 << 1,   // Backend Platform
//    supports honoring GetMouseCursor() value to change the OS cursor shape.
MAKE_ENUM(ImGuiBackendFlags_HasMouseCursors, HasMouseCursors)
//    ImGuiBackendFlags_HasSetMousePos        = 1 << 2,   // Backend Platform
//    supports io.WantSetMousePos requests to reposition the OS mouse position
...
//    supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+
...
//    ImGuiCol_Header,                // Header* colors are used for
//    CollapsingHeader, TreeNode, Selectable, MenuItem
...
//    ImGuiCol_NavHighlight,          // Gamepad/keyboard: current highlighted
//    item
...
//    ImGuiButtonFlags_MouseButtonRight       = 1 << 1,   // React on right
//    mouse button
MAKE_ENUM(ImGuiButtonFlags_MouseButtonRight, MouseButtonRight)
//    ImGuiButtonFlags_MouseButtonMiddle      = 1 << 2,   // React on center
//    mouse button
...
//    ColorEdit: disable drag and drop target. ColorButton: disable drag and
//    drop source.
...
//    ColorEdit, ColorPicker, ColorButton: display preview as a transparent
...
//    ColorEdit: override _display_ type among RGB/HSV/Hex. ColorPicker: select
...
//    0.0f..1.0f floats instead of 0..255 integers. No round-trip of value via
//    integers.
...
//    ImGuiSliderFlags_AlwaysClamp            = 1 << 4,       // Clamp value to
//    min/max bounds when input manually with CTRL+Click. By default CTRL+Click
//    allows going out of bounds.
MAKE_ENUM(ImGuiSliderFlags_AlwaysClamp, AlwaysClamp)
//    ImGuiSliderFlags_Logarithmic            = 1 << 5,       // Make the widget
//    logarithmic (linear otherwise). Consider using
...
//    rounding underlying value to match precision of the display format string
...
//    CTRL+Click or Enter key allowing to input text directly into the widget
...
//    ImGuiMouseCursor_ResizeEW,          // When hovering over a vertical
//    border or a column
...
//    ImGuiMouseCursor_Hand,              // (Unused by Dear ImGui functions.
//    Use for e.g. hyperlinks)
MAKE_ENUM(ImGuiMouseCursor_Hand, Hand)
//    ImGuiMouseCursor_NotAllowed,        // When hovering something with
//    disallowed interaction. Usually a crossed circle.
...
START_ENUM(Cond)
//    ImGuiCond_None          = 0,        // No condition (always set the
//    variable), same as _Always
MAKE_ENUM(ImGuiCond_None, None)
//    ImGuiCond_Always        = 1 << 0,   // No condition (always set the
//    variable)
MAKE_ENUM(ImGuiCond_Always, Always)
//    ImGuiCond_Once          = 1 << 1,   // Set the variable once per runtime
//    session (only the first call will succeed)
MAKE_ENUM(ImGuiCond_Once, Once)
//    ImGuiCond_FirstUseEver  = 1 << 2,   // Set the variable if the
//    object/window has no persistently saved data (no entry in .ini file)
MAKE_ENUM(ImGuiCond_FirstUseEver, FirstUseEver)
//    ImGuiCond_Appearing     = 1 << 3    // Set the variable if the
//    object/window is appearing after being hidden/inactive (or the first time)
...
//    scissoring. This is passed down to your render function but not used for
//    CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect()
...
END_IMGUI_FUNC
//    inline ImVec2   GetClipRectMin() const { const ImVec4& cr =
//    _ClipRectStack.back(); return ImVec2 cr.x  cr.y; }
// Unsupported arg type ) const { const ImVec4& cr = _ClipRectStack.back(
//    inline ImVec2   GetClipRectMax() const { const ImVec4& cr =
//    _ClipRectStack.back(); return ImVec2 cr.z  cr.w; }
// Unsupported arg type ) const { const ImVec4& cr = _ClipRectStack.back(
...
END_IMGUI_FUNC
//    IMGUI_API void  AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32
//    col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners =
//    ImDrawCornerFlags_All, float thickness = 1.0f);   // a: upper-left, b:
//    lower-right (== upper-left + size), rounding_corners_flags: 4 bits
//    corresponding to which corner to round
// Unsupported arg type  ImDrawCornerFlags rounding_corners =
// ImDrawCornerFlags_All
//    IMGUI_API void  AddRectFilled(const ImVec2& p_min, const ImVec2& p_max,
//    ImU32 col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners =
//    ImDrawCornerFlags_All);                     // a: upper-left, b:
//    lower-right (== upper-left + size)
// Unsupported arg type  ImDrawCornerFlags rounding_corners =
// ImDrawCornerFlags_All
//    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2&
//    p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32
//    col_bot_left);
...
DRAW_LIST_CALL_FUNCTION_NO_RET(AddRectFilledMultiColor, p_min, p_max,
                               col_upr_left, col_upr_right, col_bot_right,
                               col_bot_left)
END_IMGUI_FUNC
//    IMGUI_API void  AddQuad(const ImVec2& p1, const ImVec2& p2, const ImVec2&
//    p3, const ImVec2& p4, ImU32 col, float thickness = 1.0f);
...
END_IMGUI_FUNC
//    IMGUI_API void  AddQuadFilled(const ImVec2& p1, const ImVec2& p2, const
//    ImVec2& p3, const ImVec2& p4, ImU32 col);
...
END_IMGUI_FUNC
//    IMGUI_API void  AddTriangle(const ImVec2& p1, const ImVec2& p2, const
//    ImVec2& p3, ImU32 col, float thickness = 1.0f);
...
OPTIONAL_NUMBER_ARG(thickness, 1.0f)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddCircle, center, radius, col, num_segments,
                               thickness)
END_IMGUI_FUNC
//    IMGUI_API void  AddCircleFilled(const ImVec2& center, float radius, ImU32
//    col, int num_segments = 0);
IMGUI_FUNCTION_DRAW_LIST(AddCircleFilled)
IM_VEC_2_ARG(center)
NUMBER_ARG(radius)
UINT_ARG(col)
OPTIONAL_INT_ARG(num_segments, 0)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddCircleFilled, center, radius, col,
                               num_segments)
END_IMGUI_FUNC
//    IMGUI_API void  AddNgon(const ImVec2& center, float radius, ImU32 col, int
//    num_segments, float thickness = 1.0f);
...
OPTIONAL_NUMBER_ARG(thickness, 1.0f)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddNgon, center, radius, col, num_segments,
                               thickness)
END_IMGUI_FUNC
//    IMGUI_API void  AddNgonFilled(const ImVec2& center, float radius, ImU32
//    col, int num_segments);
...
END_IMGUI_FUNC
//    IMGUI_API void  AddText(const ImVec2& pos, ImU32 col, const char*
//    text_begin, const char* text_end = NULL);
...
//    pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float
//    wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
// Unsupported arg type const ImFont* font
// Unsupported arg type  const ImVec4* cpu_fine_clip_rect = NULL
//    IMGUI_API void  AddPolyline(const ImVec2* points, int num_points, ImU32
//    col, bool closed, float thickness);
...
//    clockwise order.
// Unsupported arg type const ImVec2* points
//    IMGUI_API void  AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const
...
OPTIONAL_INT_ARG(num_segments, 0)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddBezierCubic, p1, p2, p3, p4, col, thickness,
                               num_segments)
END_IMGUI_FUNC
//    IMGUI_API void  AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2,
//    const ImVec2& p3, ImU32 col, float thickness, int num_segments = 0); //
...
NUMBER_ARG(thickness)
OPTIONAL_INT_ARG(num_segments, 0)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddBezierQuadratic, p1, p2, p3, col, thickness,
                               num_segments)
END_IMGUI_FUNC
//    IMGUI_API void  AddImage(ImTextureID user_texture_id, const ImVec2& p_min,
//    const ImVec2& p_max, const ImVec2& uv_min = ImVec2 0  0, const ImVec2&
//    uv_max = ImVec2 1  1, ImU32 col = IM_COL32_WHITE);
...
//    p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2&
//    uv1 = ImVec2 0  0, const ImVec2& uv2 = ImVec2 1  0, const ImVec2& uv3 =
//    ImVec2 1  1, const ImVec2& uv4 = ImVec2 0  1, ImU32 col = IM_COL32_WHITE);
...
//    p_min, const ImVec2& p_max, const ImVec2& uv_min, const ImVec2& uv_max,
//    ImU32 col, float rounding, ImDrawCornerFlags rounding_corners =
//    ImDrawCornerFlags_All);
// Unsupported arg type  ImDrawCornerFlags rounding_corners =
// ImDrawCornerFlags_All
//    inline    void  PathLineTo(const ImVec2& pos) { _Path.push_back(pos); }
// Unsupported arg type const ImVec2& pos)                               {
// _Path.push_back(pos
//    inline    void  PathLineToMergeDuplicate(const ImVec2& pos) { if
//    (_Path.Size == 0 || memcmp(&_Path.Data[_Path.Size - 1], &pos, 8) != 0)
//    _Path.push_back(pos); }
// Unsupported arg type const ImVec2& pos)                 { if (_Path.Size == 0
// || memcmp(&_Path.Data[_Path.Size - 1] Unsupported arg type  &pos Unsupported
// arg type  8) != 0) _Path.push_back(pos
//    inline    void  PathFillConvex(ImU32 col) {
//    AddConvexPolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }  //
//    Note: Anti-aliased filling requires points to be in clockwise order.
// Unsupported arg type ImU32 col)                                   {
// AddConvexPolyFilled(_Path.Data Unsupported arg type  _Path.Size Unsupported
// arg type  col
//    inline    void  PathStroke(ImU32 col, bool closed, float thickness = 1.0f)
//    { AddPolyline(_Path.Data, _Path.Size, col, closed, thickness); _Path.Size
//    = 0; }
...
DRAW_LIST_CALL_FUNCTION_NO_RET(PathArcTo, center, radius, a_min, a_max,
                               num_segments)
END_IMGUI_FUNC
//    IMGUI_API void  PathArcToFast(const ImVec2& center, float radius, int
//    a_min_of_12, int a_max_of_12);            // Use precomputed angles for a
//    12 steps circle
...
INT_ARG(a_min_of_12)
INT_ARG(a_max_of_12)
DRAW_LIST_CALL_FUNCTION_NO_RET(PathArcToFast, center, radius, a_min_of_12,
                               a_max_of_12)
END_IMGUI_FUNC
//    IMGUI_API void  PathBezierCubicCurveTo(const ImVec2& p2, const ImVec2& p3,
//    const ImVec2& p4, int num_segments = 0);  // Cubic Bezier (4 control
//    points)
...
END_IMGUI_FUNC
//    IMGUI_API void  PathBezierQuadraticCurveTo(const ImVec2& p2, const ImVec2&
//    p3, int num_segments = 0);                // Quadratic Bezier (3 control
//    points)
...
//    float rounding = 0.0f, ImDrawCornerFlags rounding_corners =
//    ImDrawCornerFlags_All);
// Unsupported arg type  ImDrawCornerFlags rounding_corners =
// ImDrawCornerFlags_All
//    IMGUI_API void  AddCallback(ImDrawCallback callback, void* callback_data);
//    // Your rendering function must check for 'UserCallback' in ImDrawCmd and
...
//    create a new draw call (to allow for dependent rendering / blending).
//    Otherwise primitives are merged into the same draw-call as much as
//    possible
IMGUI_FUNCTION_DRAW_LIST(AddDrawCmd)
DRAW_LIST_CALL_FUNCTION_NO_RET(AddDrawCmd)
END_IMGUI_FUNC
//    inline void     ChannelsSplit(int count)    { _Splitter.Split(this,
//    count); }
...
END_IMGUI_FUNC
//    IMGUI_API void  PrimRectUV(const ImVec2& a, const ImVec2& b, const ImVec2&
//    uv_a, const ImVec2& uv_b, ImU32 col);
...
END_IMGUI_FUNC
//    IMGUI_API void  PrimQuadUV(const ImVec2& a, const ImVec2& b, const ImVec2&
//    c, const ImVec2& d, const ImVec2& uv_a, const ImVec2& uv_b, const ImVec2&
//    uv_c, const ImVec2& uv_d, ImU32 col);
...
IM_VEC_2_ARG(uv_d)
UINT_ARG(col)
DRAW_LIST_CALL_FUNCTION_NO_RET(PrimQuadUV, a, b, c, d, uv_a, uv_b, uv_c, uv_d,
                               col)
END_IMGUI_FUNC
...
//    const ImVec2& p4, int num_segments = 0) { PathBezierCubicCurveTo(p2, p3,
//    p4, num_segments); }
